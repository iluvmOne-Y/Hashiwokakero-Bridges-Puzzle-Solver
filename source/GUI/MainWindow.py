...
class MainWindow(QMainWindow):
    ...
    def solvePuzzle(self):
        ...
        try:
            ...
            if self.solution:
                ...
                if hasattr(self.level, 'solving_time'):
                    self.statusBar.showMessage(f"Solution found using {solver} in {self.level.solving_time:.4f} seconds")
                else: 
    ...
    def saveSolution(self):
        """Save the current solution automatically using the level number."""
        ...
        try:
            if self.level.number:
                # Generate the output path based on the level number
                output_filename = f"output-{self.level.number:02d}.txt"
                output_path = f"source/Outputs/{output_filename}"
                
                # Save using the FileController
                FileController.WriteSolutionToFile(self.level)
                self.statusBar.showMessage(f"Solution automatically saved to {output_path}")
                QMessageBox.information(self, "Success", f"Solution saved to {output_path}")
            else:
                QMessageBox.warning(self, "Warning", "Level number not set.")
        except Exception as e:
